...
# has ONE home -- greenhouse.resolve_values -- and Workable delegates to it rather
# than duplicating a load-bearing safety rule. Workable's `avatar` upload field is
# exactly the photo signal the rule keys on.
...
def resolve_values(fieldmap: FieldMap, ssot, profile: dict, *,
    ...
    purely on the FORM's structure, so it is correct for a schema-captured
    Workable field map exactly as for a Greenhouse one."""
