...
landed in Stage 2e WITH the eager-light `_registry.py`, because the then-live
`registry.py` (deleted in Stage 3c) loaded at `engine.providers.__init__` time
and from-imported the adapter out of the old `engine.discover` shim (dissolved
in Stage 4); the adapter's plugin home was only reachable THROUGH that same
partially-initialized package (an earlier load-time move was a proven hard
import cycle).
