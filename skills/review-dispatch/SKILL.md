...
2. **R-2 baseline injection** (no-commit-projects convention repos): stash the baseline (`git -C <repo> status --short` and `diff` to `/tmp/<session>-baseline.*`) and inject the baseline path with explicit "these files are PRIOR state, not your concern" guidance. Prevents dirty-tree false-positive REWORKs.
