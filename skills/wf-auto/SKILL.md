...
3. **Seal-revision binding**: the driver records the artifact's content manifest before dispatching the seal auditor and re-verifies it after; any mismatch voids the seal and forces a fresh seal round.
4. **Stale-seal rejection**: only the most recent seal that post-dates the last artifact change can terminate the loop, enforced by the hash check plus a monotonic round counter in the handoff file.
