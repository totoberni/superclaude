...
mod_counter() {
  ...
  CALL_COUNT=0
  [ -f "$COUNTER_FILE" ] && CALL_COUNT=$(safe_int "$(cat "$COUNTER_FILE" 2>/dev/null)")
  ...
  TDD_COUNT=0
  [ -f "$TDD_FILE" ] && TDD_COUNT=$(safe_int "$(cat "$TDD_FILE" 2>/dev/null)")
  ...
  if [[ "$TDD_COUNT" =~ ^[0-9]+$ ]] && [ "$TDD_COUNT" -ge 5 ] && [ $((TDD_COUNT % 5)) -eq 0 ]; then
    emit_context "TDD reminder: $TDD_COUNT file edits since last test run. Consider running tests before more changes. Use /test-scaffold if the project lacks test infrastructure."
    return 0
  fi

  # Efficiency check every 25 calls
  if [ $((CALL_COUNT % 25)) -eq 0 ] && [ "$CALL_COUNT" -gt 0 ]; then
    emit_context "Efficiency check ($CALL_COUNT tool calls this session). Consider: (1) delegate parallelizable work to workers, (2) use offset/limit for large files, (3) batch related operations, (4) let worker failures stand -- do not redo their work yourself."
  fi
}
